...
module.exports = function (app) {
  ...
  app.post('/api/v1/payment/subscription', async function (req, res) {
    ...
    try {
      ...
      const existingSubscription = await db.select('*').from('se_project.subscription').where({ userid: userid, subtype: subtype }).first();
      ...
      try {
        ...
        return res.status(200).json({ transactionId: newTransaction[0], subscriptionId: newSubscription[0], numTickets });
      } catch (e) {
        await client.rollback();
        console.log(e.message);
        return res.status(400).json({ message: 'Could not complete payment' });
      } finally {
  ...
  app.post('/api/v1/tickets/purchase/subscription', async (req, res) => {
    try {
      ...
      if (tripDateObj < today) {
        ...
        return res.status(400).json({ message: 'Trip date is in the past' });
      }
  
      // Check if subscription exists
      const subscription = await db.select('*').from('se_project.subscription').where({ id: subId, userid }).first();
      if (!subscription) {
      ...
      if (subscription.nooftickets < 1) {
        ...
        return res.status(400).json({ message: 'No available tickets on subscription' });
      }
  
      // Get fare zone data based on subscription zone ID
      const zone = await db.select('*').from('se_project.zones').where({ id: subscription.zoneid }).first();
      if (!zone) {
      ...
      await db('se_project.subscription').where({ id: subId }).decrement('nooftickets', 1);
  
      console.log('Ticket purchase successful');
  
      // Send response with ticket and ride data
      return res.status(200).json({ ticket: newTicket, ride: newRide });
    } catch (error) {
      console.error(error);
  ...
  app.post('/api/v1/refund/:ticketId', async function (req, res) {
    ...
    try {
      ...
      const existingTicket = await db.select('*').from('se_project.tickets').where({ id: ticketId, user_id: req.user.id }).first();
      ...
      if (existingTicket.sub_id) {
        ...
        const subscription = await db.select('*').from('se_project.subscriptions').where({ id: existingTicket.sub_id }).first();
        const zone = await db.select('*').from('se_project.zones').where({ id: subscription.zone_id }).first();
        ...
        const transaction = await db.select('*').from('se_project.transactions').where({ purchasediid: ticketId }).first();
      ...
      await db.delete().from('se_project.tickets').where({ id: ticketId });
  ...
  app.delete('/api/v1/route/:routeId', async function (req, res) {
    ...
    try {
        ...
        const existingRoute = await db.select('*').from('se_project.routes').where({ id: routeId }).first();
        ...
        await db.delete().from('se_project.routes').where({ id: routeId });
        ...
        const isStartStationConnected = await db.select('*').from('se_project.routes').where({ fromstationid: startStationId }).first();
        console.log('Is start station connected:', isStartStationConnected);  
        const isEndStationConnected = await db.select('*').from('se_project.routes').where({ tostationid: endStationId }).first(); // Changed the column name to "tostationid"
        ...
        if (!isStartStationConnected) {
            await db('se_project.stations').update({ stationstatus: 'unconnected', stationposition: endStationId }).where({ id: startStationId });
            console.log('Start station updated successfully');
        }
        if (!isEndStationConnected) {
            await db('se_project.stations').update({ stationstatus: 'unconnected', stationposition: startStationId }).where({ id: endStationId });
